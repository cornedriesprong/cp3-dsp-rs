...
mod tests {
    ...
    fn test_sequencer() {
        ...
        let size = 10.;
        let mut sequencer = Sequencer::new(size, link);
        sequencer.init();
        ...
        let mut events = HashMap::<usize, Vec<Note>>::new();
        sequencer.process(&mut events, size);

        assert_eq!(events.len(), 6);
        for (offset, ev) in events.into_iter() {
            if offset == 0 {
                assert_eq!(ev.len(), 1);
                assert_eq!(
                    ev[0],
                ...
                assert_eq!(ev[1], Note::Off { pitch: 70 });
            } else if offset == 3 {
                assert_eq!(ev.len(), 2);
                assert_eq!(
                    ev[0],
                ...
                assert_eq!(ev.len(), 1);
                assert_eq!(ev[0], Note::Off { pitch: 64 });
            } else if offset == 6 {
                assert_eq!(ev.len(), 1);
                assert_eq!(ev[0], Note::Off { pitch: 69 });
            } else {
